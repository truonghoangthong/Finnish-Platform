...
const parseFeedback = (feedback) => {
  try {
    ...
    return { grammar_feedback: feedback, vocabulary_feedback: "", overall_feedback: "", encouragement: "" };
  }
};
...
const extractQuestions = (data, part, count, hasAnswer = false) =>
  Array.from({ length: count }, (_, i) => {
    ...
    return q
      ? { id: `${part}-${i + 1}`, text: q.script, audio: q.audioBase64, ...(hasAnswer ? { answer: q.answer } : {}) }
      : null;
  }).filter(Boolean);

const toggleAudio = (part, index, questions, activeAudio, setActiveAudio, currentAudio, setCurrentAudio) => {
...
const Module4 = () => {
  ...
  const [moduleData, setModuleData] = useState({ part4a: {}, part4b: {}, part4c: {} });
  ...
  const [feedbackModal, setFeedbackModal] = useState({ show: false, part4b: [], part4c: [] });
  ...
  const handleTranslationChange = (id, v) => setTranslations((p) => ({ ...p, [id]: v }));
      ...
      const part4bResults = await Promise.all(
        moduleData.part4b.questions.map(async (q) => {
          try {
            ...
            return { question: q.text, userTranslation: translations[q.id] || "", feedback: res.data.feedback };
          } catch (e) {
            return { question: q.text, userTranslation: translations[q.id] || "", feedback: `Error: ${e.message}` };
          }
        })
      );

      setFeedbackModal({ show: true, part4b: part4bResults, part4c: part4cResults });
    } finally {
    ...
    <div className="module4-wrapper">
      <div className="module4-image-section">
        {moduleData.part4a.imgLink && (
          <img
            ...
            onError={(e) => (e.target.src = "/path-to-default-image/default-image.jpg")}
          />
        )}
        <div className="module4-mascot-container">
          <div className="module4-mascot" onClick={() => setShowFinnishInstruction(!showFinnishInstruction)}>
            <Mascot />
          </div>
          {showFinnishInstruction && (
            <div className="module4-instruction-box">
              <div className="module4-instruction-text">Klikkaa tekstiä kuunnellaksesi</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="module4-content-section">
        ...
        <div className="module4-scroll-container">
          <Title script={moduleData.part4a.title?.script} audioBase64={moduleData.part4a.title?.audioBase64} />
          {moduleData.part4a.description?.script && <p className="module4-description">{moduleData.part4a.description.script}</p>}
          <div className="module4-audio-list">
            {moduleData.part4a.questions?.map((q, i) => (
              <div
                ...
                onClick={() => toggleAudio("4a", i, moduleData.part4a.questions, activeAudio, setActiveAudio, currentAudio, setCurrentAudio)}
              >
                <span>{q.text}</span>
              </div>
            ))}
          ...
          <Title script={moduleData.part4b.title?.script} audioBase64={moduleData.part4b.title?.audioBase64} />
          ...
          <div className="module4-translation-exercise">
            {moduleData.part4b.questions?.map((q, i) => (
              <div
                ...
                onClick={() => toggleAudio("4b", i, moduleData.part4b.questions, activeAudio, setActiveAudio, currentAudio, setCurrentAudio)}
              >
          ...
          <Title script={moduleData.part4c.title?.script} audioBase64={moduleData.part4c.title?.audioBase64} />
          ...
          <div className="module4-quiz-section">
            {moduleData.part4c.questions?.map((q, i) => (
              <div
                ...
                onClick={() => toggleAudio("4c", i, moduleData.part4c.questions, activeAudio, setActiveAudio, currentAudio, setCurrentAudio)}
              >
                <p className="module4-clickable-text">{q.text}</p>
          ...
          <button className="module4-check-button" onClick={checkAnswers} disabled={checkingAnswers}>
            {checkingAnswers ? (
              <>
          ...
          <div className="module4-feedback-content">
              ...
              {feedbackModal.part4b.map((q, i) => {
                ...
                return (
                  <div key={`fb-4b-${i}`} className="module4-feedback-item">
                    <p><strong>Lause:</strong> {q.question}</p>
                    <p><strong>Sinun käännöksesi:</strong> {q.userTranslation || "(ei vastausta)"}</p>
                    {fb.grammar_feedback && <p><strong>Grammar:</strong> {fb.grammar_feedback}</p>}
                    {fb.vocabulary_feedback && <p><strong>Vocabulary:</strong> {fb.vocabulary_feedback}</p>}
                    {fb.overall_feedback && <p><strong>Overall:</strong> {fb.overall_feedback}</p>}
                    {fb.encouragement && <p className="module4-encouragement">{fb.encouragement}</p>}
                  </div>
                );
              })}
            </div>
            ...
            <div className="module4-feedback-section">
              ...
              <p className="module4-score">
                Your score: {feedbackModal.part4c.filter((r) => r.isCorrect).length}/{feedbackModal.part4c.length}
              </p>
              {feedbackModal.part4c.map((q, i) => (
                <div key={`fb-4c-${i}`} className={`module4-feedback-item ${q.isCorrect ? "module4-correct" : "module4-incorrect"}`}>
                  <p><strong>Question:</strong> {q.question}</p>
                  <p><strong>Your answer:</strong> {q.userAnswer ? "True" : "False"} | <strong>Correct:</strong> {q.correctAnswer ? "True" : "False"}</p>
                </div>
              ))}
            </div>

            <button className="module4-close-feedback" onClick={() => setFeedbackModal({ show: false, part4b: [], part4c: [] })}>
              Sulje
            </button>
          </div>
        </div>
